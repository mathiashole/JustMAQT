...
# if (length(args) < 2) {
#   stop("Uso: ./pinta_filog.R <archivo_filog> <palabra1> [<palabra2> ...]")
# }
# 
# # Archivo de filogenia y palabras a pintar
# archivo_filog <- args[1]
# palabras <- args[-1]
# 
# # Leer árbol filogenético
# tree <- read.tree(archivo_filog)
# 
# # Verificar si el árbol contiene etiquetas
# if (length(tree$tip.label) == 0) {
#   stop("El árbol no contiene etiquetas de tips.")
...
# # Generar una paleta de colores para las palabras
# n_palabras <- length(palabras)
# colores <- brewer.pal(n_palabras, "Set3")
# 
# # Pintar las ramas que contienen las palabras especificadas
# for (i in 1:n_palabras) {
#   palabra <- palabras[i]
#   data$color[grepl(palabra, data$label)] <- colores[i]
# }
# 
# # Crear un objeto ggtree
# p <- ggtree(tree)
# 
# # Añadir colores a las ramas del árbol
# p <- p %<+% data + 
...
# output_pdf <- sub("\\..+$", ".pdf", archivo_filog)
